...
server.tool("text-to-image",
  { 
    ...
    output_compression: z.number().min(0).max(100).optional().describe("输出压缩质量(0-100)。数值越高质量越好"),
    moderation: z.enum(objectValuesToZodEnum(MODERATION_LEVELS)).optional().describe("内容审核级别：low(低) 或 auto(自动)").default(MODERATION_LEVELS.LOW),
    background: z.enum(objectValuesToZodEnum(BACKGROUNDS)).optional().describe("背景类型：transparent(透明), opaque(不透明), auto(自动)").default(BACKGROUNDS.AUTO),
    quality: z.enum(objectValuesToZodEnum(QUALITIES)).optional().describe("图像质量：standard(标准), high(高), medium(中), low(低), auto(自动)").default(QUALITIES.AUTO),
